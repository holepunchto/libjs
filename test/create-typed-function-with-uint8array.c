...
uint32_t
on_typed_call(js_value_t *receiver, js_value_t *typedarray, js_typed_callback_info_t *info) {
  int e;
...
main() {
  ...
  js_value_t *script;
  e = js_create_string_utf8(env, (utf8_t *) "var i = 0, j; while (i++ < 200000) j = hello(Uint8Array.from([1, 2, 3, 4]))", -1, &script);
  assert(e == 0);
