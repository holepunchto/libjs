...
main() {
  ...
  // An exception thrown when running the script propagates as it would for a
  // script run with `js_run_script()`.
  ...
  // The handle survives a throw and can be run again.

  e = js_run_prepared_script(env, script, &result);
  assert(e == js_uncaught_exception);

  assert(uncaught_called == 2);
