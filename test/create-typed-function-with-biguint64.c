...
uint32_t
on_typed_call(js_value_t *receiver, uint64_t number, js_typed_callback_info_t *info) {
  typed_calls++;
...
main() {
  ...
  js_value_t *script;
  e = js_create_string_utf8(env, (utf8_t *) "let obj = { hello }, i = 0, j; while (i++ < 200000) j = hello(2n ** 64n - 1n)", -1, &script);
  assert(e == 0);
