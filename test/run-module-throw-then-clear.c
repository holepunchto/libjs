...
on_module_evaluate(js_env_t *env, js_module_t *module, void *data) {
  ...
  // Clearing the error is meant to leave the evaluation able to carry on, so
...
main() {
  ...
  // The export the evaluation went on to set is there, and the module can be
  // run again.
  ...
  js_value_t *value;
  e = js_get_named_property(env, namespace, "foo", &value);
  assert(e == 0);
  ...
  js_value_t *again;
  e = js_run_module(env, synthetic, &again);
  assert(e == 0);
