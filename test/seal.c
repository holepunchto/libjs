...
main() {
  ...
  e = js_seal(env, object);
  assert(e == 0);

  // A sealed object cannot have new properties added. Some engines silently
  // ignore the write while others throw; tolerate both and assert only that
  ...
  // A sealed object can still have existing properties changed.
  js_value_t *other;
